...
ColVis.prototype = {
	...
	"_fnDrawCallback": function ()
	{
		...
		for ( var i=0, iLen=aoColumns.length ; i<iLen ; i++ )
		{
			if ( this.dom.buttons[i] !== null )
			{
				//if ( aoColumns[i].bVisible && localStorage[window.location.hash+'_'+$('span .ColVis_title', this.dom.buttons[i]).text()] !== 'true')
    ...
    "_fnDomColumnButton": function ( i )
    {
        ...
        if ( sTitle == '' || sTitle.indexOf('-hidden') >0){
        ...
        $(nButton).click( function (e) {
            ...
            if ( e.target.nodeName.toLowerCase() == "input" )
            {
                ...
                $('input', this).prop('checked', showHide);
            }
            if(!showHide){
                localStorage[$("#show-shortcut-name").text()+'_' + window.location.hash+'_'+sTitle] = 'true';
            }else{
                localStorage[$("#show-shortcut-name").text()+'_' + window.location.hash+'_'+sTitle] = 'false';
            }
