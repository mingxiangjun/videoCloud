package com.video.portal.user.controller;

import com.video.portal.bean.SessionItem;
import com.video.portal.comm.Constants;
import com.video.portal.user.from.*;
import com.video.portal.user.po.UserAccountPo;
import com.video.portal.user.service.UserAccountService;
import com.video.portal.user.validate.UserValidator;
import com.video.portal.util.FormHelper;
import com.video.portal.util.RandomValidateCode;
import com.video.portal.util.ResponseCode;
import com.video.portal.util.ResponseItem;
...
/*    @InitBinder
    ...
    @ResponseBody
    public boolean logout(@ModelAttribute(Constants.SESSION_ID) SessionItem si, SessionStatus sessionStatus, HttpServletResponse res) throws Exception {
...
/*            Object p = session.getAttribute("registerPhone");
    ...
    @ResponseBody
    public ResponseItem info(@ModelAttribute(Constants.SESSION_ID) SessionItem si) throws Exception {
        ResponseItem ri = new ResponseItem();
        try {
    ...
    @ResponseBody
    public ResponseItem edit(@Valid UserEditForm form, BindingResult result, @ModelAttribute(Constants.SESSION_ID) SessionItem si) throws Exception {
        ResponseItem ri = formHelper.check(result);
        if (!ri.getCode().contains("Success")) {
            return ri;
        }
    ...
    @ResponseBody
    public ResponseItem editPwd(@Valid UserEditPwdForm form, BindingResult result, @ModelAttribute(Constants.SESSION_ID) SessionItem si) throws Exception {
        ResponseItem ri = formHelper.check(result);
        if (!ri.getCode().contains("Success")) {
            return ri;
        }
    ...
    @ResponseBody
/*    public boolean index( @ModelAttribute(Constants.SESSION_ID) SessionItem si,HttpSession session, HttpServletResponse res) throws Exception {
        try {
            res.sendRedirect(LOGIN_URL);
            // TODO 判断随机验证码
    ...
    public ModelAndView index( @ModelAttribute(Constants.SESSION_ID) SessionItem si,HttpSession session, HttpServletResponse res) throws Exception {
