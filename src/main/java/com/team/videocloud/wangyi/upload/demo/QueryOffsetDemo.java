...
import com.team.videocloud.wangyi.upload.param.*;
...
public class QueryOffsetDemo {
    ...
    public static void main(String[] args) throws VcloudException, IOException {
        ...
        if (!FileUtil.doesFileExist(filePath)) {
            log.error("[UploadVideoDemo] the path is wrong");
            return;
        }
        ...
        if (initUploadVideoParam.getCode() != 200) {
            log.error("[UploadVideoDemo] fail to init upload params. " + initUploadVideoParam.getMsg());
            return;
        }
	     /*获取上传加速节点地址*/	   
	     /*获取上传加速节点地址返回结果的封装类*/
        ...
        if (null == getUploadHostParam) {
            log.error("[UploadVideoDemo] fail to get uploadHost");
            return;
        }
	     /*分片上传视频*/      
        ...
        try {
            ...
            if (fileSize == null) {
                log.error("[UploadVideoDemo] failed to get File Length");
                return;
            }
			 /*上传文件剩余大小*/
            ...
            log.info("[UploadVideoDemo] video id :" + queryVideoIDParam.getRet().getList().get(0).getVid());
            ...
            if (null != in) {
                try {
                    ...
                    log.error("[UploadVideoDemo] fail to close inputStream " + e1.getMessage());
                }
            }
        }
    }
